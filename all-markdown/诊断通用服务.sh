...
find_log_file() {
    ...
    case "$service_type" in
        nginx)
            # Nginx 服务使用系统日志
            if [ "$port" = "8080" ]; then
                # 网站前端 - 优先使用专用日志，回退到通用日志
                if [ -f "/var/log/nginx/website-frontend-access.log" ]; then
                    log_file="/var/log/nginx/website-frontend-access.log"
                ...
                # 管理后台前端 - 优先使用专用日志，回退到通用日志
                if [ -f "/var/log/nginx/admin-frontend-access.log" ]; then
                    log_file="/var/log/nginx/admin-frontend-access.log"
                elif [ -f "/var/log/nginx/access.log" ]; then
                    log_file="/var/log/nginx/access.log"
                elif [ -f "/var/log/nginx/error.log" ]; then
                    log_file="/var/log/nginx/error.log"
    ...
    if [ -n "$SERVICE_TYPE" ]; then
        ...
        case "$SERVICE_TYPE" in
            nginx)
                ...
                echo "    - 专用日志: /var/log/nginx/website-frontend-*.log 或 /var/log/nginx/admin-frontend-*.log"
...
if [ "$SERVICE_TYPE" = "python-http-server" ]; then
    SUGGESTIONS+=("使用Python http.server，建议迁移到Nginx以获得更好的性能")
fi
